...
# Fix model paths BEFORE copying to QGIS
python3 fix_model_paths.py
if [ $? -ne 0 ]; then
    echo "[WARNING] Python script kon model paden niet aanpassen"
    echo "Models worden gekopieerd met originele paden"
fi

echo ""
echo "========================================"
echo "  3. QGIS models kopiëren"
echo "========================================"
echo ""
...
echo ""
echo "========================================"
echo "  4. Stijlbestanden kopiëren"
echo "========================================"
echo ""

STYLES_DIR="$QGIS_PROFILE/styles/tactile-maps"

# Maak styles directory aan
mkdir -p "$STYLES_DIR"

# Kopieer alle QML bestanden
if ls styles/*.qml 1> /dev/null 2>&1; then
    cp styles/*.qml "$STYLES_DIR/"
